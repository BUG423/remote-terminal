...
// ─── 模拟 createSession 的唯一性逻辑 ────────────────────────────
const VALID_STATUSES = new Set(['running', 'exited']);

function testCreateSessionAllowsSameTitleAndCwd() {
  console.log('\n📋 测试 1: createSession 允许同 title + cwd 的不同会话');
  ...
  const newSession = {
    id: 'session-2',
    title: 'Stock',
    cwd: '/home/mz/WebClaudeWorkspaces',
    status: 'running'
  };
  sessions.set(newSession.id, newSession);

  if (sessions.size !== 2 || !sessions.has('session-1') || !sessions.has('session-2')) {
    console.log(`  ❌ 同名同目录会话被错误吞掉: size=${sessions.size}`);
    return false;
  }

  console.log('  ✅ 不同 sessionId 的同名同目录会话可并存');
...
// ─── 测试同 title+cwd 不同 ID 保留 ───────────────────────────────
function testListSessionsKeepsSameTitleAndCwd() {
  console.log('\n📋 测试 3: listSessions 保留同 title+cwd 的不同 ID');

  const sessions = new Map();
  // 两个 running 会话，同 title+cwd 但不同 ID，是合法的多终端场景。
  sessions.set('s1', { id: 's1', title: 'Stock', cwd: '/ws/Stock', pid: 100, status: 'running', createdAt: 1 });
  sessions.set('s2', { id: 's2', title: 'Stock', cwd: '/ws/Stock', pid: 200, status: 'running', createdAt: 2 });
  ...
  const ids = filtered.map(s => s.id);
  const expectedIds = ['s1', 's2', 's3'];

  if (ids.length === expectedIds.length && ids.every((id, i) => id === expectedIds[i])) {
    console.log(`  ✅ 保留正确: ${ids}`);
  } else {
    console.log(`  ❌ 保留错误: 期望 ${expectedIds}, 实际 ${ids}`);
    return false;
  }
...
function testEdgeCases() {
  ...
  const emptyFiltered2 = emptyFields2.filter(s => {
    if (!['running', 'exited', 'disconnected'].includes(s.status)) return false;
    if (seen3.has(s.id)) return false;
    seen3.add(s.id);
    return true;
  });
...
// ─── 运行所有测试 ─────────────────────────────────────────────────
console.log('╔══════════════════════════════════════════╗');
console.log('║   Session 唯一性 & 过滤 单元测试         ║');
console.log('╚══════════════════════════════════════════╝');

const results = [
  testCreateSessionAllowsSameTitleAndCwd(),
  testListSessionsFiltering(),
  testListSessionsKeepsSameTitleAndCwd(),
